...
func (b *Bulk) runInsert(action *bulkAction, result *BulkResult, berr *BulkError) bool {
	...
	defer func() {
		...
		if duration > 1*time.Second {
			logf("Bulk insert operation takes more than 1 sec [%v %v]", b.c.FullName, action.docs)
		}
		mongoDurationTimeMs.WithLabelValues("bulkinsert").Observe(duration.Seconds() * 1000.0)
	}()
	numMongoReqs.WithLabelValues("bulkinsert").Inc()
	...
	if err != nil {
		numMongoErrors.WithLabelValues("bulkinsert").Inc()
	}
...
func (b *Bulk) runUpdate(action *bulkAction, result *BulkResult, berr *BulkError) bool {
	...
	defer func() {
		duration := time.Since(start)
		if duration > 1*time.Second {
			logf("Bulk update operation takes more than 1 sec [%v %v]", b.c.FullName, action.docs)
		}
		mongoDurationTimeMs.WithLabelValues("bulkupdate").Observe(duration.Seconds() * 1000.0)
	}()
	numMongoReqs.WithLabelValues("bulkupdate").Inc()
	...
	if err != nil {
		numMongoErrors.WithLabelValues("bulkupdate").Inc()
	}
	return b.checkSuccess(action, berr, lerr, err)
}
...
func (b *Bulk) runRemove(action *bulkAction, result *BulkResult, berr *BulkError) bool {
	...
	defer func() {
		duration := time.Since(start)
		if duration > 1*time.Second {
			logf("Bulk remove operation takes more than 1 sec [%v %v]", b.c.FullName, action.docs)
		}
		mongoDurationTimeMs.WithLabelValues("bulkremove").Observe(duration.Seconds() * 1000.0)
	}()
	numMongoReqs.WithLabelValues("bulkremove").Inc()
	...
	if err != nil {
		numMongoErrors.WithLabelValues("bulkremove").Inc()
	}
	return b.checkSuccess(action, berr, lerr, err)
}
